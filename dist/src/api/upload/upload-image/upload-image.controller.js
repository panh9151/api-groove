...
let UploadImageController = class UploadImageController {
    ...
    async deleteImage(body) {
        ...
        const public_id = `${matches[2]}`.replace(/\.(jpg|jpeg|png|gif|webp)$/i, "");
        if (!public_id)
    ...
    (0, common_1.UseInterceptors)((0, platform_express_1.FileInterceptor)("file", {
        fileFilter: (req, file, callback) => {
            ...
            if (!allowedMimeTypes.includes(file.mimetype)) {
                return callback(new common_1.BadRequestException("Only image files (jpeg, jpg, png, gif, webp) are allowed!"), false);
            }
            callback(null, true);
        },
    })),
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], UploadImageController.prototype, "deleteImage", null);
